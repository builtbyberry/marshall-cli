...
/**
 * Resolve how this CLI talks to the Marshall store.
 *
...
/**
 * The backend values that opt a repo into the Marshall store. `"srm"` is a
 * literal other repos already hold in their tracked config, so it can NEVER be
 * dropped from this set — but `"marshall"` is equally valid, so a new repo never
 * has to write the retired product name into its own config.
 *
 * This is a WIDENING, not a rename. A rename (drop old, add new) would silently
 * stop recognising every repo already opted in; accepting both breaks nobody.
 * The env surface has worked this way all along — pick() reads MARSHALL_* and
 * falls back to SRM_* — and `state.backend` was simply the one identifier that
 * never got the treatment.
 */
const OPTED_IN_BACKENDS = new Set(['srm', 'marshall']);

/**
 * Assert this REPO opted into the Marshall store — `state.backend` is `"srm"` or
 * `"marshall"` in its .claude/release-config.json.
 *
...
export function requireRepoOptIn(config) {
    if (!OPTED_IN_BACKENDS.has(config.backend)) {
        throw new Error(
            'This repo does not use Marshall (`state.backend` is not "srm" or ' +
                '"marshall" in its .claude/release-config.json). Nothing to do.',
        );
    }
}
